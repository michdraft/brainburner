...
public class UsersFrame extends Frame {
	...
	public UsersFrame(DBConnection connection, LoginFrame lframe) {
		initComponents();
    ...
    private void btn_createActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_createActionPerformed
	    ...
	    int id = Users.getUser(connection, username).getId();


	    if (!Helpers.cmpPasswords(password, password2)) {
		    Messages.showWarning("Passwords do not match!");
	    } else {
		    User user = new User(username, new String(password), id);
		    ...
		    if (user.insert(connection)) {
			    ...
			    this.loginFrame.fillLoginMask(username, new String(password));
		    }
	    }
